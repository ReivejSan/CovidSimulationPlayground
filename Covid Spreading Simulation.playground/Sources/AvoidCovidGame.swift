...
public class AvoidCovidGame: SKScene, SKPhysicsContactDelegate {
    //VIP CONSTANT
...
//    }
    func title() {
        let title = SKLabelNode(text: "Avoid Virus & Infected Person")
        title.fontName = "AvenirNext-Bold"
        title.fontSize = 75
        title.position = CGPoint(x: frame.midX, y: frame.midY + 600.0)
        title.zPosition = 2
...
//        uiButton.clipsToBounds = true
        ...
        //add Player
                player = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "normal person.png")), color: .clear, size: CGSize(width: size.width * 0.05, height: size.width * 0.05)) ///normalperson.png
    ...
    func triggerGameOver() {
        ...
        gameOverLabel.position = CGPoint(x: frame.midX, y: 100)
