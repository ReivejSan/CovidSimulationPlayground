...
public class WithMask: SKScene, SKPhysicsContactDelegate {
    //VIP CONSTANT
    let numberOfPeople = 10
    let numberInfectedPeople = 10
    let numberOfCovid = 5
...
//        uiButton.clipsToBounds = true
        ...
        for _ in 1 ... numberInfectedPeople {
            createInfectedPerson()
        }
        
        for _ in 1 ... numberOfCovid {
            createEnemy()
        }
...
//        maskedPerson.texture = SKTexture(image: #imageLiteral(resourceName: "image-removebg-preview-3.png"))
...
//        infectedEnemy.texture = SKTexture(image: #imageLiteral(resourceName: "image-removebg-preview-5.png"))
//        infectedEnemy.physicsBody?.categoryBitMask = BitMask.enemy
//        (infectedEnemy.children.first as? SKShapeNode)?.strokeColor = .red
    }
    
    func createEnemy() {
        let covid = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "virus.png")), color: .clear, size: CGSize(width: size.width * 0.05, height: size.width * 0.05)) ///virus.png
        
        covid.position = CGPoint(x: positionWithin(range: 1, containerSize: size.width), y: positionWithin(range: 1, containerSize: size.height))
        
        covid.addCircle(radius: minimumLenght, edgeColor: .orange, filled: false)
        
        addChild(covid)
        people.append(covid)
        print("Enemy spawned")
        covid.physicsBody = SKPhysicsBody(circleOfRadius: covid.size.width * (0.5 + minimumLenght))
        covid.physicsBody?.affectedByGravity = false
        covid.physicsBody?.categoryBitMask = BitMask.enemy
        covid.physicsBody?.contactTestBitMask = BitMask.unmaskedPerson
        covid.physicsBody?.friction = 0.0
        covid.physicsBody?.angularDamping = 0.0
        covid.physicsBody?.restitution = 1.1
        covid.physicsBody?.allowsRotation = false
    }
    
    func createPerson() {
        
        let person = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "image-removebg-preview-3.png")), color: .clear, size: CGSize(width: size.width * 0.05, height: size.width * 0.05)) ///normalperson.png
    ...
    func createInfectedPerson() {
        let infectedPerson = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "normal person.png")), color: .clear, size: CGSize(width: size.width * 0.05, height: size.width * 0.05)) ///sickperson.png
        
        infectedPerson.position = CGPoint(x: positionWithin(range: 1, containerSize: size.width), y: positionWithin(range: 1, containerSize: size.height))
        
        infectedPerson.addCircle(radius: minimumLenght, edgeColor: .orange, filled: false)
        
        addChild(infectedPerson)
        people.append(infectedPerson)
        print("Unmasked spawned")
        infectedPerson.physicsBody = SKPhysicsBody(circleOfRadius: infectedPerson.size.width * (0.5 + minimumLenght))
        infectedPerson.physicsBody?.affectedByGravity = false
        infectedPerson.physicsBody?.categoryBitMask = BitMask.unmaskedPerson
        infectedPerson.physicsBody?.contactTestBitMask = BitMask.enemy
        infectedPerson.physicsBody?.friction = 0.0
        infectedPerson.physicsBody?.angularDamping = 0.0
        infectedPerson.physicsBody?.restitution = 1.1
        infectedPerson.physicsBody?.allowsRotation = false
    }
    ...
    public func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.categoryBitMask == BitMask.unmaskedPerson && contact.bodyB.categoryBitMask == BitMask.enemy {
            infect(person: contact.bodyA.node as! SKSpriteNode)
            
        } else if contact.bodyB.categoryBitMask == BitMask.unmaskedPerson && contact.bodyA.categoryBitMask == BitMask.enemy {
    ...
    func infect(person: SKSpriteNode) {
        person.texture = SKTexture(image: #imageLiteral(resourceName: "1__#$!@%!#__sick person.png")) ///sickperson.png
        person.physicsBody?.categoryBitMask = BitMask.unmaskedPerson
        (person.children.first as? SKShapeNode)?.strokeColor = .red
    }
    
    func triggerGameOver() {
        gameOver = true
        player.texture = SKTexture(image: #imageLiteral(resourceName: "image-removebg-preview-4.png")) ///sickperson.png
